...
test("Testing increaseQuantity & decreaseQuantity methods", () => {
    ...
    expect(cart.items).toEqual([gadgetApple, gadgetSamsung]);
    expect(cart._items.reduce( (sum, item) => sum + item.quantity, 0 )).toBe(3);        // проверку НЕ проходит
    // expect(gadgetApple.quantity).toBe(2);                                               // проверку проходит
    // expect(gadgetSamsung.quantity).toBe(1);                                             // проверку проходит
    // expect(gadgetSamsung.quantity).toBe(1);                                             // проверку проходит
})